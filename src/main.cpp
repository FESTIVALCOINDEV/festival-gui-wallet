...
// Copyright (c) 2018 The Festivalcoin developers
...
int main(int argc, char* argv[]) {
	...
	QSettings festivalcoinKey("HKEY_CLASSES_ROOT\\Festivalcoin", QSettings::NativeFormat);
	festivalcoinKey.setValue(".", "Festivalcoin Wallet");
	festivalcoinKey.setValue("URL Protocol", "");
	QSettings festivalcoinOpenKey("HKEY_CLASSES_ROOT\\festivalcoin\\shell\\open\\command", QSettings::NativeFormat);
	festivalcoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
	...
	//as root
	args << "-c" << "printf '[Desktop Entry]\\nName = FEST URL Handler\\nGenericName = festivalcoin\\nComment = Handle URL Sheme festivalcoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/festivalcoin;\\nIcon = Festivalcoin-Wallet' | tee /usr/share/applications/Festivalcoin-handler.desktop";
